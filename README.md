...
***ПРИМЕР КОНФИГА*** *(сохраняйте конкретно этот порядок, и сохраняем в формате .cfg)*
...
(через запятую указываем что использовать полсе окончания отправки последнего сообшения в диалоге)
